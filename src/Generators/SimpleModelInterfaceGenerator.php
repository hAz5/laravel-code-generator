...
class SimpleModelInterfaceGenerator extends CodeGenerator
{
    /**
     * @param string $model
     * @param array  $columns
    ...
    public function create(string $model, array $columns, $bundle): void
    {
        foreach ($columns as $key => $column) {
            $columns[$key]['upper'] = Str::of($column['fieldName'])->upper();
        ...
        $generated = view(
            'CG::samples.SimpleModelInterface',
            [
                'bundle' => $bundle,
                'model' => $model,
                'columns' => $columns,
        ...
        file_put_contents($directoryPath . '/' . Str::studly($model) . 'Interface.php', $generated);
    }
}
